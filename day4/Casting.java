...
public class Casting {
    public static void main(String[] args) {
        ...
        int a = 10; // 형변환 x
        double b = 10; // 형변환 o // int가 double로 바뀜 // 자바가 바꿔줌(자동형)

        int c = (int)3.14; // 형변환 o // double이 int로 바뀌려면 소수점 떼야됨. (수동형변환)
        System.out.println(c);

        // 객체타입 형변환
        // 자바 상속 → 상위 타입을 바탕으로 하위 타입 생성 // 예시) 동물을 바탕으로 고양이 생성
        ...
        Cat c2 = (Cat)a2; // 자동 형변환 x // a2에는 Cat과 Dog가 들어있을 수 있기 때문에 Cat이라고 확인할 수 없다.
        c2.meow(); // 상위타입을 하위타입으로 바꾸는 건 위험하기 때문에 개발자가 직접 수동형변환을 해야함

        // 전혀 상관없는 객체끼리는 형변환 불가
        // 객체간 형변환는 상속관계일때 가능
